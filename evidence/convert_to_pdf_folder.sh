...
# Convert all evidence markdown files to PDF in pdf/ subdirectory
# Usage: ./convert_to_pdf_folder.sh
...
PDF_DIR="pdf"
if [ ! -d "$PDF_DIR" ]; then
    echo -e "${BLUE}Creating ${PDF_DIR}/ directory...${NC}"
    mkdir -p "$PDF_DIR"
    echo ""
fi
...
# List generated PDFs
if [ $count -gt 0 ]; then
    echo -e "\n${YELLOW}Generated PDFs in ${BLUE}${PDF_DIR}/${YELLOW}:${NC}"
